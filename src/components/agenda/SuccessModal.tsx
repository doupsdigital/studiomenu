    ...
    <div className="fixed inset-0 z-[300] flex items-center justify-center p-4 bg-black/45 backdrop-blur-sm" role="dialog" aria-modal="true">
      <div className="bg-surface rounded-2xl border border-linen shadow-2xl w-full max-w-md overflow-hidden">
        <div className="px-6 pt-6 pb-2 text-center">
          ...
          <p className="text-sm text-ink-soft mt-1">Os dados foram registrados com sucesso no sistema.</p>
        </div>

        <div className="px-6 py-4 space-y-4">
          <div className="bg-cream border border-linen rounded-xl p-4 text-[15px] space-y-2.5">
            {rows.map((row, i) => (
              <div
                key={row.label}
                className={`flex justify-between ${i < rows.length - 1 ? 'border-b border-linen pb-1.5' : ''}`}
              >
                <span className="font-bold text-ink-soft uppercase text-[13px] tracking-wider">{row.label}</span>
          ...
          <button
            type="button"
            onClick={onClose}
            className="w-full py-3 rounded-lg text-[15px] font-semibold bg-rose-600 hover:bg-rose-800 text-white transition-colors"
          >
            Concluir e Fechar
          </button>
        </div>
