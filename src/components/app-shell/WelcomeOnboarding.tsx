...
import { Sparkles, Share2, CalendarClock, Calendar, type LucideIcon } from 'lucide-react';
 ...
 *  contato importa mais do que num app nativo comum.
 *
...
export const WelcomeOnboarding: React.FC<WelcomeOnboardingProps> = ({ slug, planTier }) => {
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-6 py-8 max-w-sm mx-auto w-full text-center">
        <div className="w-full rounded-2xl bg-white border border-rose-100 shadow-sm px-6 py-8">
          <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-rose-700 via-rose-600 to-rose-500 text-white flex items-center justify-center mx-auto mb-4 shadow-lg shadow-rose-600/25">
            <slide.icon className="w-8 h-8" />
          </div>
          <p className="font-serif-pro font-bold text-xl text-ink mb-1.5">{slide.title}</p>
          <p className="text-[15px] text-ink-soft leading-snug">{slide.subtitle}</p>
        </div>
      </div>
