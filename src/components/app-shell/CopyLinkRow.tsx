...
export const CopyLinkRow: React.FC<CopyLinkRowProps> = ({ label, path }) => {
  ...
  return (
    <div className="rounded-2xl bg-surface border border-linen p-4">
      <p className="text-[10px] font-semibold tracking-widest uppercase text-ink-faint mb-1.5">{label}</p>
      <div className="flex items-center gap-2">
        <p className="flex-1 min-w-0 truncate text-xs text-ink-soft font-mono">{displayUrl}</p>
        <button
          type="button"
          onClick={handleCopy}
          className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[11px] font-bold shrink-0 transition-colors ${
            copied ? 'bg-emerald-100 text-emerald-700' : 'bg-rose-100 text-rose-600 hover:bg-rose-200'
          }`}
        >
