...
function initTileObserver() {
  ...
  document.querySelectorAll('.servico-card, .tile').forEach((tile) => {
...
window.renderLista = renderGrid;

// Renderizar Lista Clássico
function renderGrid() {
  ...
  lista.forEach((item, index) => {
    ...
    card.innerHTML = `
      ...
      <div class="servico-card__conteudo">
        <span class="servico-card__cat">${getCatLabel(item)}</span>
